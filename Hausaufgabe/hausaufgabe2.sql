...
select * from kitap where kitap.sayfasayisi>200;

-- SORU02: yazar no 45 olan kitabin sayisi yazdirin
select * from yazar where yazar.yazarno=45;

-- SORU03: yazar no su 4 ile 9 arasinda olan yazar isimleriniz yazdirin 
-- and yerine && de kullanilabilir
-- or yerine | veya || yazilabilri
select yazar.ad,yazar.soyad from yazar where yazar.yazarno between '4' and '9';
select * from yazar where yazar.yazarno>='4' and yazar.yazarno<='9';


-- SORU03: charles dickens ile cengiz aytmatov arasindaki yazarlari listeleyin
select * from yazar where yazar.yazarno between '4' and '9';
select * from yazar where yazar.ad between 'Charles' and 'Cengiz';-- aski tablosuna göre siralama yapiyor tabloya göre degil alfabetik olarak arada kalanlari getirir. ch' dan sonra gelen harflere bakiyor
select * from yazar where yazar.ad between 'Cengiz' and 'Charles';-- aski tablosuna göre siralama yapiyor tabloya göre ch' dan sonra gelen harflere bakiyor
SELECT * FROM yazar
WHERE yazarno BETWEEN 
    (SELECT yazarno FROM yazar WHERE ad = 'Charles') AND (SELECT yazarno FROM yazar WHERE ad = 'Cengiz');

-- SORU04: sayfa sayisi 200 den buyuk kitap ismi 'beyaz dis' olan kitabin ismi puanini getir
select kitap.ad, kitap.puan from kitap where kitap.sayfasayisi>200 and kitap.ad='beyaz dis';

-- SORU05: kitap no su 98 54 21 76 olan kitapno ve isimlerini ylistlele
select kitap.kitapno, kitap.ad from kitap where kitap.kitapno in(98, 54, 21, 76);

-- SORU06: İsmi A harfi ile başlayan yazarlari listele
select * from yazar where yazar.ad like 'A%';
select * from yazar where regexp_like(yazar.ad,'^A','c');

-- SORU07: İsminin 2. harfi E olan personeli listeleyin
select * from yazar where yazar.ad like '_E%';

-- SORU08: İsminin 2. harfi E olup diğer harflerinde Y olan yazar
select * from yazar where yazar.ad like '_E%Y%';

-- SORU09: isim ilk harfi r ile baslasin ikini hardi ne olursa olssun icinde k harfi bulunan isimleri listele
select * from yazar where yazar.ad like 'R%k%';

-- SORU10: isminde a harfi olmayan 
select * from yazar where yazar.ad not like 'a';

-- SORU11: kitab adi 6 haneli olan kitablari listeleyin
select * from kitap where kitap.ad like '______';

-- SORU12: 1. harfi A ve 8. harfi A olan kitap listeleyin
select * from kitap where kitap.ad like 'A______A%';
