...
INSERT INTO calisanlar VALUES(123456710, 'Ercan Bak', 'Bursa', 25000, 'Trendyol');
...
select isim, maas, isyeri from calisanlar where isyeri in  -- ana sorgu
(select marka_isim from markalar where calisan_sayisi>15000);  -- alt sorgu-> sarti veren markalar table
...
select marka_isim,calisan_sayisi,(select sum(maas) from calisanlar where isyeri=marka_isim) from markalar;
select marka_isim,calisan_sayisi,(select sum(maas) from calisanlar where isyeri=marka_isim) as maas_toplam from markalar;
-- as maas_toplam-> sub sorguda olusan column icin alias (as) isim atamasi yapildi
...
select marka_isim,calisan_sayisi,(select round(avg(maas)) from calisanlar where isyeri=marka_isim) as ortalama_maas from markalar;-- round () yuvarlam islemi yapiyor 
...
(select max(maas) from calisanlar where isyeri = marka_isim) as max_maas
from markalar;
