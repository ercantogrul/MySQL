-- primary key null olamaz, unique olmak zorunda , her tabloda olmak zorunda degil, olursa da bir tane olabilir
-- unique kel null olabilir, bir tabloda birden fazla unique key olabilir.
create table Books (
...
select * from books where Title='animal_farm';
select Author from books where regexp_like(title, '^Animal') and regexp_like(title, 'farm$');
Select author from books where Title='animal_farm';
select Author from books where title like 'Animal_Farm';

-- Task 2:Native Son kiatbının id si kaçtır
select Book_id from books where title like 'Native_Son';
...
delete from books where title like'lolita';
...
barrow_book int,
constraint book_fk foreign key(barrow_book) references books(book_id)
);
...
from books,members 
where members.Name ='Ali Koc' and members.barrow_book=books.Book_id;  -- veya
select * from books,members where members.Name ='Ali Koc' and members.barrow_book=books.Book_id;

-- Task 2: 557 numaralı kitap kim tarafından ve hangi mail ile ödünc alinmistir 
select books.Title, members.Name,members.Email from books,members where book_id= 557 and books.Book_id=members.barrow_book;

-- Task 3: Fatma Çifçi adlı üyeyi listeden siliniz
delete from members where members.Name='Fatma Çiftçi';
