...
VALUES
  ...
  -- task1 müşterilerin isimlerini ve şehirlerini  tek satırda birleştirerek yazdırınız arada iki nokta  işereti ekleyiniz (edited) 
select 
concat (customerName,' : ',city) as müsteri_sehir
from customer;
...
select customername, max(orderdate) as son_order
from customer
left join orders
on customer.CustomerID=orders.CustomerID
...
select customername, orders.OrderDate
from customer
left join orders
on customer.CustomerID=orders.CustomerID
...
select city, group_concat(CustomerName separator' , ') as musteriler, sum(OrderAmount) as toplam_siparis
from orders
left join customer
on customer.CustomerID=orders.CustomerID
group by City;
